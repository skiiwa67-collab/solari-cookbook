...
export async function sparklePad(canvas) {
  if (!canvas || matchMedia("(prefers-reduced-motion: reduce)").matches) return false;
...
const canvas = document.getElementById("padgpu");
sparklePad(canvas)
  .then((ok) => {
